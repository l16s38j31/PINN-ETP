# -*- coding: utf-8 -*-
"""
Created on Mon Jul 21 08:37:15 2025
...
# PSO算法主函数
def particle_swarm_optimization(T_out, P, T_in_true, initial_T_i, initial_T_e, n_particles=50, max_iterations=100, early_stop_patience=10, early_stop_threshold=1e-5):
    # 参数范围
    param_bounds = [
        (1.0, 10.0),  # R1: °C/kW
        (0.1, 1.0),   # R2: °C/kW
    ...
    particles = np.array([[np.random.uniform(low, high) for low, high in param_bounds] for _ in range(n_particles)])
    velocities = np.zeros((n_particles, n_params))
    
    # 初始化个体和全局最优
    pbest_positions = particles.copy()
    pbest_fitness = np.array([fitness_function(p, T_out, P, T_in_true, initial_T_i, initial_T_e) for p in particles])
    ...
    for iteration in range(max_iterations):
        ...
        for i in range(n_particles):
            ...
            for j in range(n_params):
                particles[i, j] = np.clip(particles[i, j], param_bounds[j][0], param_bounds[j][1])
        
        # 计算适应度
        fitness = np.array([fitness_function(p, T_out, P, T_in_true, initial_T_i, initial_T_e) for p in particles])
...
best_params = particle_swarm_optimization(T_out, P, T_in, initial_T_i, initial_T_e)
